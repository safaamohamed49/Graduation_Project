...
<template>
  <EntityFormShell
    page-title="إضافة موظف"
    hero-badge="الموظفين / إضافة"
    hero-title="إضافة موظف جديد"
    hero-description="أدخلي بيانات الموظف والراتب والفرع، ويمكنك إنشاء حساب دخول له وتحديد صلاحيات خاصة فوق الدور الأساسي."
    ...
    <div class="grid gap-5 p-6 md:grid-cols-2">
      ...
      <div class="md:col-span-2 rounded-[26px] bg-indigo-50 p-5 ring-1 ring-indigo-100">
        <div class="flex items-center gap-3">
          <input id="create_user" v-model="form.create_user" type="checkbox" />
          <label for="create_user" class="text-sm font-black text-indigo-800">
            إنشاء حساب دخول لهذا الموظف
          </label>
        </div>
      </div>
      ...
      <template v-if="form.create_user">
        ...
        <div>
          <label class="mb-2 block text-sm font-black text-slate-700">كلمة المرور</label>
          <FormControl v-model="form.password" type="password" placeholder="6 أحرف على الأقل" />
        ...
        <div v-if="props.canManagePermissions" class="md:col-span-2 rounded-[28px] bg-white p-5 ring-1 ring-slate-200">
          <div class="mb-4">
            ...
            <p class="mt-1 text-sm font-bold text-slate-500">
              الدور يعطي صلاحيات أساسية، ويمكنك هنا إضافة صلاحيات أو منع صلاحيات لهذا المستخدم فقط.
            </p>
          </div>
